...
describe('pawn.isAttack() function', () => {
    let pawn = new Pawn('TEST', 'light', 'A2', 'TEST'); //only the team and location are needed to test this function, so the other fields are left blank
    ...
    //test spec for TRUE result
    testCases.forEach(tc => {
        //update the pawn's team and location to match the current test case
        pawn.setTeam(tc.team);
        pawn.setLocation(tc.oldSq);

        it(`Should say pawn.isAttack(${tc.newSq}) === true`, () => {
            chai.expect(pawn.isAttack(tc.newSq)).to.be.ok;
        });
    });
});
